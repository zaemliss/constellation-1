...
class RewardsNormalizerTest
  ...
  "normalizes rewards" in {
    ...
    val rewardsBalances = deserializedSnapshotInfo.addressCacheData
      .mapValues(_.rewardsBalance)

    val nonRewardsBalances = deserializedSnapshotInfo.addressCacheData.transform {
      case (address, data) => data.balance - rewardsBalances(address).toDouble
    }
